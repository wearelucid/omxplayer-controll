...
var player = function(path, options) {
  ...
  var args = path;
  if (['hdmi', 'local', 'both'].indexOf(settings.audioOutput) != -1 || settings.audioOutput.startsWith('alsa'))
  ...
  if (settings.fontPath && settings.fontPath !== '')
    args.push('--font', '' + settings.fontPath + '');
  if (settings.fontPathItalic && settings.fontPathItalic !== '')
    args.push('--italic-font', '' + settings.fontPathItalic + '');
  if (settings.fontSize)
    args.push('--font-size', '' + settings.fontSize + '');
  if (settings.fontAlign && settings.fontAlign !== '')
    args.push('--align', '' + settings.fontAlign + '');
  if (settings.videoFifo)
    args.push('--video_fifo', settings.videoFifo);
  if (settings.lines)
    args.push('--lines', settings.lines);
  if (settings.videoQueue)
    args.push('--video_queue', settings.videoQueue);
